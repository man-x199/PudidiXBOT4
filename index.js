...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285807107404@s.whatsapp.net"] // replace this with your number
			const adminbotnumber = ["6285807107404@s.whatsapp.net"]
			const frendsowner = ["62818056730972@s.whatsapp.net"]
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nama bot* : ${me.name}\n*Another* : *PudidiX*\n*FRIENDS* : Zizi-kun\n*Nomor Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : 08:00 - 22:00`
			...
			case 'creator':
                 ...
                 client.sendMessage(from, '_*itu pacar ku eh owner ku ><*_',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
                ...
                case 'quotemaker':
					...
					client.sendMessage(from, buffer, image, {caption: 'Nih tod', quoted: mek})
					break
				case 'galaxtext':
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
			    ...
			    case 'nekonime':
				    try {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ingat! *Cintai Kucingmu>~<*'})
					} catch (e) {
			    ...
			    case 'randomanime':
				    try {
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'nih randomanime!'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
               ...
               case 'infogempa':
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, '*Usernamenya mana om?*', text, {quoted: mek})
				...
				case 'tulis':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Hati-hati ketahuan guru mampus lu*'})
					break
				case 'text3d':
			    ...
			    case 'infonomor':
			        ...
			        break
			    case 'igstalk':
					if (args.length < 1) return reply('*Masukan username target stalking mu!!*')
					ige = body.slice(9)
					reply(mess.wait)
				...
				case 'url2img':
					...
					if (args.length < 1) return reply('*Tipenya apa om?*')
					if (!tipelist.includes(args[0])) return reply('Tipe *desktop|tablet|mobile*')
					if (args.length < 2) return reply('*Urlnya mana om?*')
				...
				break
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`_*Perintah Diterima*_ , _*berhasil membuka grub*_`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`_*Perintah Diterima*_ , _*berhasil menutup grub*_`)
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 *Mention All* 〙✪══'+teks+'╚═〘 *PudidiXBOT* 〙', members_id, true)
					break
				case 'clearall':
					if (!isOwner) return reply('Kamu siapa? Perintah ini khusus owner tod-,-')
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ _*PudidiX - Broadcast*_ ]\n\n${body.slice(4)}`})
						}
						reply('_Sukses broadcast_')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ _*PudidiX - Broadcast*_ ]\n\n${body.slice(4)}`)
						}
						reply('_Sukses broadcast_')
					}
					break
				...
				case 'delete':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Perintah di terima, menghapus pesan :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Perintah di terima, menghapus pesan : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.deleteMessage(from, mentioned)
					}
					break
			    case 'unpromote':
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Udah aktif om')
						welkom.push(from)
		        ...
		        case 'setpp2':
                    ...
                    break
                case 'bc2':
					if (!isadminbot) return reply('*Kamu siapa?* *Perintah ini khusus owner tod-,-*')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
                ...
                case 'bc3':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *PudidiXt - Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
